...
# 服务端 mockup 渲染用的无头 Chromium。服务器没有 GPU，靠 SwiftShader 软件渲染跑
# WebGL。注意 Alpine 的 chromium 主包**不含** SwiftShader —— 必须单独装
# chromium-swiftshader，否则 --use-angle=swiftshader 起不来，引擎会在初始化时崩。
